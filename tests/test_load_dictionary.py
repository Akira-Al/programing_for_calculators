...
def test_load_dictionary1():
    ...
    input_file = os.path.join(
        project_root_path, "tests", "test_load_dictionary_files", "test_dictionary1.txt"
    )
    expected_output = {
...
def test_load_dictionary2():
    ...
    """
    input_file = os.path.join(
        project_root_path, "tests", "test_load_dictionary_files", "test_dictionary2.txt"
    )
    expected_output = {
        "あがく": [(["あがく"], -1, "経験")],
        "あきらめる": [(["あきらめる"], -1, "経験")],
...
def test_load_PnJa():
    ...
    """
    input_file = os.path.join(
        project_root_path, "tests", "test_load_dictionary_files", "test_pn_ja.txt"
    )
    expected_output = {
        "優れる": (1.0, "すぐれる", "動詞"),
        "良い": (0.999995, "よい", "形容詞"),
