from ..src.my_library import load_dictionary
...
def test_load_dictionary1():
    ...
    """
    input_file = (
        Path(__file__).parent / "test_load_dictionary_files" / "test_dictionary1.txt"
    )
    expected_output = {
...
def test_load_dictionary2():
    ...
    """
    input_file = (
        Path(__file__).parent / "test_load_dictionary_files" / "test_dictionary2.txt"
    )
    expected_output = {
        "あがく": [(["あがく"], -1, "経験")],
        "あきらめる": [(["あきらめる"], -1, "経験")],
...
def test_load_PnJa():
    ...
    """
    input_file = Path(__file__).parent / "test_load_dictionary_files" / "test_pn_ja.txt"
    expected_output = {
        "優れる": (1.0, "すぐれる", "動詞"),
        "良い": (0.999995, "よい", "形容詞"),
        "喜ぶ": (0.999979, "よろこぶ", "動詞"),
