...
def test_load_dictionary1():
    ...
    expected_output = {'２，３日': (0, '〜である・になる（状態）客観'), '10%': (0, '〜である・になる（状態）客観'), '100%': (0, '〜である・になる（状態）客観'), '25%': (0, '〜である・になる（状態）客観'), '80%': (0, '〜である・になる（状態）客観')}
...
def test_load_dictionary2():
    ...
    expected_output = {'あがく': [(-1, ['あがく'], '経験')], 'あきらめる': [(-1, ['あきらめる'], '経験')], 'あきる': [(-1, ['あきる'], '経験')], 'あきれる': [(-1, ['あきれる'], '経験'), (-1, ['あきれる', 'た'], '経験')]}
...
def test_load_PnJa():
    ...
    expected_output = {'優れる': (1.0, 'すぐれる', '動詞'), '良い': (0.999995, 'よい', '形容詞'), '喜ぶ': (0.999979, 'よろこぶ', '動詞'), '褒める': (0.999979, 'ほめる', '動詞'), 'めでたい': (0.999645, 'めでたい', '形容詞')}
